...
import { Card, Form, Input, InputNumber, Select, Button, message, Space, Row, Col, Typography, Divider, Tag, Rate, Progress, Steps, Timeline, Modal, Badge, Spin, Alert, Collapse, Checkbox } from 'antd';
                  ...
                  {constraintOptions.map((option) => (
                    <Card
                      ...
                      style={{
                        cursor: 'pointer',
                        borderRadius: 8,
                        border: selectedConstraints.includes(option.value) ? `2px solid ${option.color}` : '1px solid #e8e8e8',
                        transition: 'all 0.3s ease'
                      }}
            ...
            >
              <Space direction="vertical" style={{ width: '100%' }} size={12}>
                {optimizationSuggestions.map((suggestion, index) => (
                  <Card key={index} size="small" className="card" style={{ borderLeft: `3px solid ${getOptimizationColor(suggestion.type)}` }}>
